...
void Emitter2D::_bind_methods() {
    ...
    ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "volley_rate", PROPERTY_HINT_RANGE, "0.1,100,0.1"), "set_volley_rate", "get_volley_rate");
    ...
    ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "spawn_offset", PROPERTY_HINT_RANGE, "0,100,0.1"), "set_spawn_offset", "get_spawn_offset");
...
void Emitter2D::do_volley() {
    for (uint32_t i = 0; i < bullets_per_volley; i++) {
        ...
        Vector2 start_pos = get_position() + (Vector2(cos(bullet_angle), sin(bullet_angle)) * spawn_offset);

        if (bullet != nullptr) {  // pool returns nullptr if no bullets left.
            bullet->start(get_settings(), bullet_angle, start_pos, this);
        }
    }
}
